...
const photos = ref([]); // เก็บข้อมูล picslider
...
const selectedImageToDelete = ref(""); // เก็บ URL รูปที่ต้องการลบ

// ดึงข้อมูล picslider จาก API
const fetchPhotos = async () => {
  try {
    const response = await axios.get(API_PATH.GET_PROFILE_SLIDER);
...
// อัปเดตรูป picslider (เพิ่มรูปใหม่)
const updatePhotos = async () => {
  ...
  try {
    ...
    // เรียก API อัปเดต
    await axios.put(API_PATH.UPDATE_PROFILE_SLIDER, formData, {
...
const deletePhoto = async () => {
  try {
    await axios.delete(API_PATH.UPDATE_PROFILE_SLIDER, {
...
<template>
    <v-row>
      <v-col cols="12">
        ...
        <!-- Modal ยืนยันการลบ -->
        <v-dialog v-model="showDeleteModal" max-width="400px" class="font-prompt">
          <v-card>
