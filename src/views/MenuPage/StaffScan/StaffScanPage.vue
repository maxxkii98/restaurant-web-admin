...
const updateStatus = async () => {
    ...
    try {
        // ดึงข้อมูล username และ role จาก LocalStorage
        const username = localStorage.getItem("username");
        const role = localStorage.getItem("role");

        if (!username || !role) {
            message.value = "ไม่พบข้อมูลผู้ใช้งาน";
            return;
        }

        // ส่งคำขออัปเดตสถานะพร้อมข้อมูล username และ role
        ...
        await fetchBookings(); // โหลดข้อมูลใหม่หลังจากอัปเดตสำเร็จ
...
<template>
        ...
        <v-card elevation="1" class="mb-8 p-0">
            <v-card-item>
                    ...
                    <v-col cols="12" lg="6" md="6">
                        <div class="d-flex pb-3">
                            ...
                            <div>
                                ...
                                <h6 class="text-sm text-gray-600">จำนวนการจองทั้งหมด</h6>
                            </div>
                        </div>
            ...
            <v-dialog v-model="showDialog" persistent max-width="500px">
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        <v-text-field v-model="enteredBookingId" label="ไอดีการจอง" variant="outlined"
                            dense></v-text-field>
                    </v-card-text>
                    <v-card-actions>
