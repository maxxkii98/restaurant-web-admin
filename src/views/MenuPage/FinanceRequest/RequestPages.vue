            ...
            <v-dialog v-model="dialog" max-width="500">
                <template v-slot:activator="{ props }">
                    <v-btn color="primary" v-bind="props" rounded="pill">
                        <v-icon class="mr-2">mdi-plus</v-icon>Create Request
                    </v-btn>
                </template>
                <v-card>
                        ...
                        <tbody class="font-prompt font-normal">
                            <tr v-for="item in paginatedItems" :key="item._id">
                                <td class="text-subtitle-1">
                                    {{ typeof item.destination === 'object' && item.destination.accountName ? item.destination.accountName : 'N/A' }}
                                </td>
...
const fetchRequests = async () => {
    try {
        const response = await axiosInstance.get<Request[]>(API_PATH.GET_REQUEST);
        requests.value = response.data.map((request) => ({
            ...request,
            destination: typeof request.destination === 'object'
                ? request.destination
                : { _id: '', accountName: 'Unknown' },
        }));
    } catch (error) {
...
const totalPages = computed(() => {
    return Math.ceil(requests.value.length / itemsPerPage.value);
});
...
const save = async () => {
    try {
        ...
        const payload = {
            destination: typeof editedItem.value.destination === "string" ? editedItem.value.destination : editedItem.value.destination._id,
            amount: Number(editedItem.value.amount),
            remark: editedItem.value.remark.trim(),
        };

        let response: AxiosResponse<Request>;

        if (editedIndex.value > -1) {
            response = await axiosInstance.put<Request>(
                `${API_PATH.ADD_REQUEST}/${editedItem.value._id}`,
                payload
            );
            Object.assign(requests.value[editedIndex.value], response.data);
        } else {
            response = await axiosInstance.post<Request>(API_PATH.ADD_REQUEST, payload);
...
const formTitle = computed(() => {
    return editedIndex.value === -1 ? "Create Request" : "Edit Request";
});
